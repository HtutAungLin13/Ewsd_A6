...
function normalizeDepartmentStatRow($row) {
    $total_staff = max(0, intval($row['total_staff'] ?? 0));
    $staff_submitted = max(0, intval($row['staff_submitted'] ?? 0));
    $staff_not_submitted = max(0, intval($row['staff_not_submitted'] ?? 0));

    if ($staff_submitted > $total_staff) {
        $staff_submitted = $total_staff;
    }
    if ($staff_not_submitted > $total_staff) {
        $staff_not_submitted = $total_staff;
    }

    // Keep totals realistic even when cached table data is stale or overcounted.
    if ($staff_submitted + $staff_not_submitted !== $total_staff) {
        $staff_not_submitted = max(0, $total_staff - $staff_submitted);
    }

    $row['total_staff'] = $total_staff;
    $row['staff_submitted'] = $staff_submitted;
    $row['staff_not_submitted'] = $staff_not_submitted;
    $row['submission_rate'] = $total_staff > 0
        ? round(($staff_submitted / $total_staff) * 100, 2)
        : 0.0;

    $row['total_ideas'] = max(0, intval($row['total_ideas'] ?? 0));
    $row['total_comments'] = max(0, intval($row['total_comments'] ?? 0));
    $row['total_votes'] = max(0, intval($row['total_votes'] ?? 0));

    return $row;
}

function getSubmittedStaffBySessionSql() {
    return "SELECT DISTINCT sub.session_id, sub.staff_id
            FROM (
                SELECT sst.session_id, sst.staff_id
                FROM staff_submission_tracking sst
                JOIN staff st ON st.id = sst.staff_id
                WHERE sst.has_submitted = 1 AND st.is_active = 1
                UNION
                SELECT i.session_id, s.id as staff_id
                FROM staff s
                JOIN contributors c
                  ON LOWER(TRIM(COALESCE(c.email, ''))) = LOWER(TRIM(COALESCE(s.email, '')))
                JOIN ideas i ON i.contributor_id = c.id
                WHERE s.is_active = 1
                  AND LOWER(TRIM(COALESCE(i.department, ''))) = LOWER(TRIM(COALESCE(s.department, '')))
            ) sub";
}

function getDepartmentActiveStaffCount($helper, $department) {
    if (trim($department) === '') {
        return 0;
    }
    ...
    return max(0, intval($row['count'] ?? 0));
}

function getDepartmentSubmittedStaffCount($helper, $department, $session_id) {
    if (trim($department) === '' || intval($session_id) <= 0) {
        return 0;
    }

    $submitted_sql = getSubmittedStaffBySessionSql();
    $row = $helper->fetchOne(
        "SELECT COUNT(DISTINCT s.id) as count
         FROM staff s
         JOIN ($submitted_sql) ss
           ON ss.staff_id = s.id
          AND ss.session_id = ?
         WHERE s.department = ? AND s.is_active = 1",
        [intval($session_id), $department]
    );
...
try {
    ...
    switch ($action) {
        ...
        case 'staff_not_submitted':
            ...
            $rows = $helper->fetchAll(
                "SELECT s.id as staff_id, s.name, s.email, s.department,
                 ...
                 FROM staff s
                 JOIN sessions ses ON ses.status IN ('Active', 'Closed')
                 LEFT JOIN ($submitted_sql) ss ON ss.staff_id = s.id AND ss.session_id = ses.id
                 $where
                 AND s.is_active = 1
        ...
        case 'staff_for_invite':
            ...
            $rows = $helper->fetchAll(
                "SELECT s.id as staff_id, s.name, s.email, s.department,
                        ses.id as session_id, ses.session_name, ses.closes_at, ses.final_closure_date,
                        DATEDIFF(COALESCE(ses.final_closure_date, ses.closes_at), NOW()) as days_until_closure,
                        CASE WHEN ss.staff_id IS NULL THEN 0 ELSE 1 END as has_submitted
                 FROM staff s
                 JOIN sessions ses ON ses.id = ?
                 LEFT JOIN ($submitted_sql) ss
                    ON ss.staff_id = s.id AND ss.session_id = ses.id
                 WHERE s.department = ?
                   AND s.is_active = 1
        ...
        case 'department_stats':
            ...
            if (empty($stats)) {
                $stats = $helper->fetchAll(
                    "SELECT sst.session_id, sst.department,
                            ...
                            COUNT(DISTINCT CASE WHEN sst.has_submitted = 1 THEN sst.staff_id END) as staff_submitted,
                            COUNT(DISTINCT CASE WHEN sst.has_submitted = 0 THEN sst.staff_id END) as staff_not_submitted,
                            COALESCE(MAX(ia.total_ideas), 0) as total_ideas,
                            COALESCE(MAX(ia.total_comments), 0) as total_comments,
                            COALESCE(MAX(ia.total_votes), 0) as total_votes,
                            COALESCE(ROUND((COUNT(DISTINCT CASE WHEN sst.has_submitted = 1 THEN sst.staff_id END) / NULLIF(COUNT(DISTINCT sst.staff_id), 0) * 100), 2), 0) as submission_rate,
                            s.session_name, s.closes_at, s.status
                     FROM staff_submission_tracking sst
                     LEFT JOIN (
                        SELECT i.session_id, i.department,
                               COUNT(DISTINCT i.id) as total_ideas,
                               COALESCE(SUM(COALESCE(i.comment_count, 0)), 0) as total_comments,
                               COALESCE(SUM(COALESCE(i.upvote_count, 0) + COALESCE(i.downvote_count, 0)), 0) as total_votes
                        FROM ideas i
                        GROUP BY i.session_id, i.department
                     ) ia ON ia.session_id = sst.session_id AND ia.department = sst.department
            ...
            if (empty($stats) && $session_id > 0) {
                $session_meta = $helper->fetchOne(
                    "SELECT id as session_id, session_name, closes_at, status
                     FROM sessions
                     WHERE id = ?
                     LIMIT 1",
                    [$session_id]
                ...
                if ($session_meta) {
                    $idea_totals = $helper->fetchOne(
                        "SELECT COUNT(DISTINCT i.id) as total_ideas,
                                COALESCE(SUM(COALESCE(i.comment_count, 0)), 0) as total_comments,
                                COALESCE(SUM(COALESCE(i.upvote_count, 0) + COALESCE(i.downvote_count, 0)), 0) as total_votes
                         FROM ideas i
                         WHERE i.session_id = ? AND i.department = ?",
                        [$session_id, $department]
                    );

                    $stats[] = [
                        'session_id' => intval($session_meta['session_id']),
                        'department' => $department,
                        'total_staff' => 0,
                        'staff_submitted' => 0,
                        'staff_not_submitted' => 0,
                        'total_ideas' => intval($idea_totals['total_ideas'] ?? 0),
                        'total_comments' => intval($idea_totals['total_comments'] ?? 0),
                        'total_votes' => intval($idea_totals['total_votes'] ?? 0),
                        'submission_rate' => 0,
                        'session_name' => $session_meta['session_name'],
                        'closes_at' => $session_meta['closes_at'],
            ...
            $active_staff_cache = [];
            $submitted_staff_cache = [];
            foreach ($stats as &$stat) {
                $row_department = trim(strval($stat['department'] ?? $department));
                $row_session_id = intval($stat['session_id'] ?? 0);

                if ($row_department !== '' && $row_session_id > 0) {
                    if (!array_key_exists($row_department, $active_staff_cache)) {
                        $active_staff_cache[$row_department] = getDepartmentActiveStaffCount($helper, $row_department);
                    }
                    $submitted_key = $row_department . '|' . $row_session_id;
                    if (!array_key_exists($submitted_key, $submitted_staff_cache)) {
                        $submitted_staff_cache[$submitted_key] = getDepartmentSubmittedStaffCount(
                            $helper,
                            $row_department,
                            $row_session_id
                        );
                    ...
                    $stat['total_staff'] = $active_staff_cache[$row_department];
                    $stat['staff_submitted'] = $submitted_staff_cache[$submitted_key];
                    $stat['staff_not_submitted'] = max(
                        0,
                        intval($stat['total_staff']) - intval($stat['staff_submitted'])
                    );
                }

                $stat = normalizeDepartmentStatRow($stat);
            }
            unset($stat);
