...
try {
    switch ($action) {
        ...
        case 'export_csv':
            ...
            $columns = [
                'id', 'title', 'description', 'department', 'impact_level',
                'status', 'approval_status', 'is_inappropriate',
                'contributor', 'email', 'is_anonymous',
                'submitted_at', 'like_count', 'comment_count',
                'category', 'session_name', 'tags'
            ];
        ...
        case 'export_zip':
            ...
            if ($zip_size === false || $zip_size <= 0) {
                echo ApiResponse::error('ZIP archive is empty or unreadable', 500);
                exit();
            }
