...
try {
    ...
    switch ($action) {
        ...
        case 'anonymous_content':
            if ($method === 'GET') {
                // Use submission-level anonymity as source of truth.
                // Fallback to contributor-level anonymous flag for older data.
                $idea_has_is_anonymous = columnExists($helper, 'ideas', 'is_anonymous');
                $comment_has_is_anonymous = columnExists($helper, 'comments', 'is_anonymous');
                $comment_has_is_deleted = columnExists($helper, 'comments', 'is_deleted');

                $idea_anonymous_where = $idea_has_is_anonymous
                    ? "i.is_anonymous = 1 OR con.is_anonymous = 1"
                    : "con.is_anonymous = 1";

                $comment_anonymous_where = $comment_has_is_anonymous
                    ? "c.is_anonymous = 1 OR con.is_anonymous = 1"
                    : "con.is_anonymous = 1";

                $comment_visible_where = $comment_has_is_deleted ? "AND c.is_deleted = 0" : "";
                $comment_count_visible_join = $comment_has_is_deleted
                    ? "LEFT JOIN comments c ON i.id = c.idea_id AND c.is_deleted = 0"
                ...
                $anonymous_ideas = $helper->fetchAll(
                    "SELECT i.id, 'Idea' as type, i.title, i.department, i.created_at,
                     ...
                     FROM ideas i
                     LEFT JOIN contributors con ON i.contributor_id = con.id
                     $comment_count_visible_join
                     WHERE $idea_anonymous_where
                     GROUP BY i.id, i.title, i.department, i.created_at
                ...
                $anonymous_comments = $helper->fetchAll(
                    "SELECT c.id, 'Comment' as type, c.content as title, i.title as idea_title, c.created_at,
                     ...
                     JOIN ideas i ON c.idea_id = i.id
                     WHERE ($comment_anonymous_where) $comment_visible_where
                     ORDER BY c.created_at DESC"
                );
        ...
        case 'unresponded_ideas':
            if ($method === 'GET') {
                $comment_has_is_deleted = columnExists($helper, 'comments', 'is_deleted');
                $comment_join = $comment_has_is_deleted
                    ? "LEFT JOIN comments cm ON cm.idea_id = i.id AND cm.is_deleted = 0"
                ...
                $ideas = $helper->fetchAll(
                    "SELECT i.id, i.title, i.department, i.created_at, i.comment_count,
                     ...
                     $comment_join
                     GROUP BY i.id, i.title, i.department, i.created_at, i.comment_count, c.name, cat.name
