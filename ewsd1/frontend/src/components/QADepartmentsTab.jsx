...
export default function QADepartmentsTab({ onError, searchTerm = '' }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [deptRes, coordRes] = await Promise.all([
        apiClient.get('/qa_management.php', { params: { action: 'departments' } }),
        apiClient.get('/qa_management.php', { params: { action: 'coordinators' } }),
      ]);
  ...
  const submit = async (event) => {
    ...
    try {
      if (editing) {
        await apiClient.put('/qa_management.php?action=departments', {
          ...formData,
          qa_coordinator_id: formData.qa_coordinator_id ? Number(formData.qa_coordinator_id) : 0,
        });
      } else {
        await apiClient.post('/qa_management.php?action=departments', {
          ...formData,
          qa_coordinator_id: formData.qa_coordinator_id ? Number(formData.qa_coordinator_id) : 0,
        });
      }
  ...
  const removeDepartment = async (departmentId) => {
    ...
    try {
      await apiClient.delete('/qa_management.php?action=departments', { data: { department_id: departmentId } });
      await fetchData();
    } catch (error) {
